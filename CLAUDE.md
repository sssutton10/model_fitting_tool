...
- **Derived variables with `input_cols`** — when creating a variable from a different source column (e.g. "region" from "state"), use `input_cols=["state"]` not `col="state"`. The first positional arg to `add_variable()` is always the output variable name AND the `col` param. Exactly one input without `custom_transform` creates a named scalar built-in pipeline; binning, one-hot encoding, and polynomial expansion are not supported for these aliases. With `custom_transform`, one or more inputs are accepted. Categorical detection is automatic from the transform's output dtype (string → categorical, numeric → continuous); `is_categorical=True` is only needed to force categorical treatment when the output dtype is numeric.
- **Chained derived variables** — `input_cols` may name other registered derived variables. Pipeline aliases provide their fitted scalar value downstream; custom-derived variables provide their raw callable result before their own cap/log/bin/encoding pipeline. Only variables explicitly requested by the model are emitted as predictors.
