...
def _bin_for_plot(
    s: pl.Series, n_bins: int | None = 10, breaks: list[float] | None = None
) -> pl.Series:
    """
    ...
    labeled = (
        s_float.cut(list(breaks), labels=all_labels, left_closed=True)
        .cast(pl.Utf8)
        .fill_null("Missing")
    )
...
def _bar_with_line(
    ...
    if ref_line is not None:
        ax2.axhline(
            ref_line, color="black", linewidth=1.0, linestyle="--", alpha=0.55, zorder=2
        )
...
def ae_chart(
    ...
    actual_rel, pred_rel = _weighted_relativity(
        summary["prod_y"], summary["exposure"], summary["prod_pred"]
    )
    ...
    ax2 = ax.twinx()
    ax2.plot(
        x, actual_rel, marker="o", linewidth=2.5, markersize=7, label="Actual", zorder=3
    )
    ax2.plot(
        x,
        pred_rel,
...
def double_lift_chart(
    ...
    fig.suptitle(
        f"Double Lift: {name1} vs {name2}", fontsize=13, fontweight="bold", y=1.01
    )
    fig.tight_layout()
    return fig
...
def decile_lift_chart(
    ...
    plot_data = (
        tbl.select(["bucket", "actual", "predicted"])
        .unpivot(index="bucket")
        .to_pandas()
    )
...
def midpoint_movement_histogram(
    ...
    breaks = [
        i / 100 for i in range(-55, 60, 5)
    ]  # -0.55, -0.50, ..., 0.55  (23 breaks → 24 buckets)
    ...
    ax.set_title(
        title or "Midpoint Movement Distribution", fontsize=14, fontweight="bold"
    )
...
def coefficient_plot(
    ...
    """
    df = (
        coef_df.filter(pl.col("feature") != "intercept")
...
def cv_stability_plot(
    ...
    fold_rows = stability_df.filter(~pl.col("fold").is_in(["geomean", "std", "cv_pct"]))
    if fold_rows.is_empty():
        raise ValueError("stability_df contains no fold rows.")

    feat_cols = [c for c in fold_rows.columns if c != "fold"]
    means_abs = {
        c: float(fold_rows[c].cast(pl.Float64).abs().mean() or 0.0) for c in feat_cols
    }
...
def metrics_bar_chart(
    ...
    show = [
        m for m in ["rmse", "mae", "gini_norm"] if m in metrics_df[metric_col].to_list()
    ]
    ...
    for ax, metric in zip(axes, show):
        ...
        bars = ax.bar(
            list(version_cols), vals, color=colors[: len(version_cols)], alpha=0.85
        )
...
def interaction_heatmap(
    ...
    all_vars = sorted(set(df["var1"].to_list()) | set(df["var2"].to_list()))
...
def pd_plot_2d(
    ...
    ax.set_title(
        f"2D Partial Dependence: {var1_name} x {var2_name}",
        fontsize=12,
        fontweight="bold",
    )
    fig.tight_layout()
...
def importance_plot(
    ...
    ax.set_title(
        title or f"Permutation Importance — top {len(variables)}",
        fontsize=11,
        fontweight="bold",
    )
    ax.grid(axis="x", alpha=0.3)
...
def _residual_heatmap_summary(
    ...
    """Aggregate weighted actual, predicted, exposure, and A/E by two levels."""
    work = pl.DataFrame(
        {
            "_level1": level1,
            "_level2": level2,
            "_actual": actual,
            "_pred": pl.Series("_pred", predictions),
            "_w": weights,
        }
    )
    return (
        work.group_by(["_level1", "_level2"])
        .agg(
            [
                ((pl.col("_actual") * pl.col("_w")).sum() / pl.col("_w").sum()).alias(
                    "actual_mean"
                ),
                ((pl.col("_pred") * pl.col("_w")).sum() / pl.col("_w").sum()).alias(
                    "pred_mean"
                ),
                pl.col("_w").sum().alias("exposure"),
            ]
        )
        .with_columns((pl.col("actual_mean") / pl.col("pred_mean")).alias("ae_ratio"))
    )


def _residual_heatmap_matrices(
    summary: pl.DataFrame,
) -> tuple[list[str], list[str], np.ndarray, np.ndarray]:
    """Convert a two-level residual summary to naturally ordered matrices."""
    labels1 = _sort_labels(summary["_level1"].unique().to_list())
    labels2 = _sort_labels(summary["_level2"].unique().to_list())
    index1 = {value: index for index, value in enumerate(labels1)}
    index2 = {value: index for index, value in enumerate(labels2)}
    ae_matrix = np.full((len(labels2), len(labels1)), np.nan)
    exposure_matrix = np.zeros((len(labels2), len(labels1)))
    for row in summary.iter_rows(named=True):
        column = index1.get(row["_level1"])
        line = index2.get(row["_level2"])
        if column is not None and line is not None:
            ae_matrix[line, column] = row["ae_ratio"]
            exposure_matrix[line, column] = row["exposure"]
    return labels1, labels2, ae_matrix, exposure_matrix


def _residual_heatmap_annotations(
    ae_matrix: np.ndarray,
    exposure_matrix: np.ndarray,
) -> np.ndarray:
    """Build the two-line A/E and exposure annotation for every matrix cell."""
    annotations = np.empty(ae_matrix.shape, dtype=object)
    for row in range(ae_matrix.shape[0]):
        for column in range(ae_matrix.shape[1]):
            annotations[row, column] = (
                f"{ae_matrix[row, column]:.2f}\n({exposure_matrix[row, column]:,.0f})"
                if not np.isnan(ae_matrix[row, column])
                else ""
            )
    return annotations
...
def residual_heatmap(
    ...
    summary = _residual_heatmap_summary(level1, level2, y, predictions, w)
    labels1, labels2, ae_matrix, exposure_matrix = _residual_heatmap_matrices(summary)
    fig, ax = plt.subplots(
        figsize=figsize or (max(8, len(labels1) * 0.9), max(6, len(labels2) * 0.8))
    )
    annot_matrix = _residual_heatmap_annotations(ae_matrix, exposure_matrix)
    ...
    ax.set_title(
        title or f"2D Residual Heatmap: {col1} x {col2}", fontsize=12, fontweight="bold"
    )
    fig.tight_layout()
...
def overfitting_plot(
    ...
    ax.plot(
        steps, cv, color="#e6550d", marker="s", linewidth=2, markersize=6, label="CV"
    )
    ax.fill_between(
        steps, train, cv, alpha=0.15, color="#e6550d", label="Gap (overfitting)"
    )
...
def bootstrap_ci_plot(
    ...
    ax.set_title(
        title or "Bootstrap Confidence Intervals", fontsize=11, fontweight="bold"
    )
    ax.grid(axis="x", alpha=0.3)
    fig.tight_layout()
    return fig
...
def relativities_ci_plot(
    ...
    ax.bar(
        x, rels, yerr=errors, color="#3182bd", alpha=0.85, capsize=4, ecolor="#333333"
    )
    ...
    ax.set_title(
        f"Bootstrap Relativity CIs — {variable}", fontsize=11, fontweight="bold"
    )
    ax.legend(fontsize=9)
    ax.grid(axis="y", alpha=0.3)
    fig.tight_layout()
